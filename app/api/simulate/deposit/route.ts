// app/api/simulate/deposit/route.ts
// POST /api/simulate/deposit { from, collateralAmount, borrowAmount }
// Uses Tenderly REST Simulation API against real Base Sepolia (network_id: 84532).
// Simulation state matches real chain — sees user's actual XAUT approval and KYC.
...
export async function POST(req: NextRequest) {
    try {
        const { from, collateralAmount, borrowAmount } = await req.json()
        if (!from || !collateralAmount || !borrowAmount) {
        ...
        const input = encodeFunctionData({
            ...
            args: [BigInt(collateralAmount), BigInt(borrowAmount)],
        })

        // Tenderly REST Simulation API — simulates against real Base Sepolia state
        const res = await fetch(TENDERLY_URL, {
            ...
            body: JSON.stringify({
                ...
                gas_price: '0',
                value: '0',
                save: true,           // visible in Tenderly dashboard
                save_if_fails: true,
            }),
        })
        ...
        if (!tx?.status) {
            // Extract revert reason from Tenderly trace
            const reason =
        ...
        return NextResponse.json({
            ...
            revertReason: null,
            simulationId: data.simulation?.id,   // link to Tenderly dashboard
        })
