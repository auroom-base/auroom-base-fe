...
interface BenefitItem {
    ...
    description: string;
    image: string;
    iconType: 'zap' | 'percent' | 'shield' | 'globe' | 'smartphone' | 'refresh';
}

interface BenefitsAnimatedProps {
    benefits: BenefitItem[];
    comparisonData: Array<{ feature: string; auroom: string; traditional: string }>;
}

const iconMap = {
    zap: <Zap className="w-6 h-6 text-[#F5A623]" />,
    percent: <BadgePercent className="w-6 h-6 text-[#F5A623]" />,
    shield: <ShieldCheck className="w-6 h-6 text-[#F5A623]" />,
    globe: <Globe className="w-6 h-6 text-[#F5A623]" />,
    smartphone: <Smartphone className="w-6 h-6 text-[#F5A623]" />,
    refresh: <RefreshCw className="w-6 h-6 text-[#F5A623]" />,
};

/**
 * Client component for BenefitsSection animations.
 * Receives static data from server component.
 */
export function BenefitsAnimated({ benefits, comparisonData }: BenefitsAnimatedProps) {
